...
IF~Global("g-knowgirgosz","GLOBAL",0)~THEN REPLY @3 /*"Greetings. I just saw you staring at the water. Who are you?" */ DO~SetGlobal("g-knowgirgosz","GLOBAL",1)~ + g-bbD058.who2
...
@26 /*"Ask away." */
END
IF~Global("G-rahomon-G","GLOBAL",0) Global("g-yagnobetray","GLOBAL",0) Global("g-girgoszwater","GLOBAL",1) Global("G-rahomon-F","GLOBAL",1) Global("G-rahomon-W","GLOBAL",1)~THEN REPLY @28 /*"There was an incident a couple cycles ago... between a marraenoloth and a band of wererats. Did you maybe witness that during your nightly observation?" */ DO~~ + g-bbD058.rashomon
IF~Global("G-rahomon-G","GLOBAL",0) Global("g-yagnobetray","GLOBAL",1) Global("g-girgoszwater","GLOBAL",1) Global("G-rahomon-F","GLOBAL",1) Global("G-rahomon-W","GLOBAL",1)~THEN REPLY @29 /*"There was an incident a couple cycles ago... between a marraenoloth and a band of wererats. Did you maybe witness that during your nightly observation?" */ DO~~ + g-bbD058.rashomonalt
IF~Global("G-knowAcademy","GLOBAL",1)~THEN REPLY @30 /*"How is your dissertation going? Any progress?" */ DO~~ + g-bbD058.academicupdate
...
IF~Global("g-girgoszguv","GLOBAL",0) ~THEN REPLY @36 /*"You mentioned you were one of the Guvners? What can you tell me about them?" */ DO~SetGlobal("g-girgoszguv","GLOBAL",1) ~ + g-bbD058.guv
IF~Global("g-girgoszguv","GLOBAL",1) ~THEN REPLY @37 /*"Can you tell me about the Guvners again?" */ DO~~ + g-bbD058.guvre
IF~Global("g-girgoszwater","GLOBAL",0) ~THEN REPLY @38 /*"What are you doing here, staring at the water?" */ DO~SetGlobal("g-girgoszwater","GLOBAL",1) ~ + g-bbD058.what
...
@42 /*"Ah. I am searching for evidence for the existence of Cassie, the Ditch Beast." He pauses and steps closer to the edge of the cliff. */
==DAnnah IF ~InParty("Annah") !StateCheck("Annah",CD_STATE_NOTVALID) NearbyDialog("DAnnah")~ THEN @43 /*"Oh bother, that nonsense again." */
==g-bbd058 @44 /*"The rumors about her have been circulating for decades, and there are as many people who claim to have seen her as there are those who claim Cassie *cannot* be real. The Fraternity of Order just cannot have that, can we?" */
==g-bbd058 @45 /*"Cassie must be massive, so she should be easily spotted as the banks of this 'water' reservoir are full of people, yet she only ever seems to present itself in the dead of anti-peak, in most remote locations, or, presumably leaves no witnesses behind, which is frustrating. How can we catalog and taxonomize such an elusive beast?" */
END
...
==DMorte IF ~InParty("Morte") !StateCheck("Morte",CD_STATE_NOTVALID) NearbyDialog("DMorte")~ THEN @56 /*"And just like that, I fell asleep." Morte starts fake-snoring. */
==g-bbd058 @57 /*"I represent the Department of Creature Taxonomy, and this project is really my dissertation at the Rowan Academy Of Learning." */
END
IF~Global("G-knowRowan","GLOBAL",1) Global("G-dissrowan","GLOBAL",1) Global("G-knowAcademy","GLOBAL",0)~THEN REPLY @59 /*Lie: "Oh, not the Rowan Academy! I've heard it's a nest of corruption and pseudoscience." */ DO~SetGlobal("G-knowAcademy","GLOBAL",1) IncrementGlobal("G-fatedpower","GLOBAL",-1) IncrementGlobal("LAW","GLOBAL",-1)~ + g-bbD058.guv2a
...
@109 /*Girgosh looks at the thick waters. "Well... I just spent the night seeing mostly nothing. Now I can at least observe the changes that happened on the coast, on the islands and among the debris while it was dark." */
END
...
==G-bbd058 IF~Global("g-girgoszannah","GLOBAL",1)~ THEN @117 /*The man looks at Annah, as if afraid of her reaction, and starts cautiously. "The beast... is called Cassie and she is rumored to roam the Ditch at night. Of course *many*... " He looks to the tiefling again. "Many deny her existence, but she attained a legend status in the city so it would be great to learn something concrete." */
...
==DAnnah IF ~InParty("Annah") !StateCheck("Annah",CD_STATE_NOTVALID) NearbyDialog("DAnnah")~ THEN @119 /*"I'm watching yeh." */
END
...
@130 /*"Oh, quite a few by the Seafarer's Arch, but that is some distance away from here." */
END
...
@142 /*"Oh, then the influence would be much weaker. Anyway, I do not think a hopping portal would appear next to the Arch. From what I've heard, hopping portals don't like to hop near permanent portals." */
END
...
@147 /*"Oh... well, in fact, I did witness the whole thing. The yagnoloths have questioned me about it, but I did not see where the rats went apart from just disappearing underwater." */
END
...
@151 /*"Oh... well, in fact, I did witness the whole thing. The yagnoloths have questioned me about it, but I did not see where the rats went apart from just disappearing underwater... However, I heard the fiends found them and killed them." */
END
...
=@156 /*"There was some shuffling about and then I think the creature at the shore was killed, turned to *dust.* The wererats bolted then, splitting up with the marraenoloth, it would seem, who shouted some things and walked away." */
END
